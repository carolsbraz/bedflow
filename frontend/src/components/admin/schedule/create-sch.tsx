...
import { GetUsers, User } from "../../../services/users.service.ts";
...
export function CreateScheduleDialog({
  ...
  const handleSave = async () => {
    const createAndNotify = async (schedule: Schedule) => {
      try {
    ...
    if (recurrent === "sim") {
      ...
      const checkedDaysSet = new Set(checkedDays);

      for (
        let currentDate = new Date(newSchedule.day);
        currentDate <= endDate;
        currentDate.setDate(currentDate.getDate() + 1)
      ) {
        if (checkedDaysSet.has(currentDate.getDay())) {
          ...
          let schedule = {
            ...
            day: formattedDate,
            hour: newSchedule.startTime,
            endHour: newSchedule.endTime,
          };

          await createAndNotify(schedule);
        }
      }
    } else {
      let schedule = {
        ...
        <Dialog.Content className="fixed overflow-hidden overflow-y-auto inset-0 md:inset-auto md:right-0 md:top-0 md:max-w-[640px] py-10 px-10 w-full md:h-full bg-white flex flex-col outline-none">
          ...
          <form className="flex-1 flex flex-col">
            <div className="flex flex-col mt-5">
              <label htmlFor="userSelect" className="text-slate-700 mb-2">
                Usuário
              </label>
              <select
                id="userSelect"
                value={newSchedule.selectedUser || ""}
                onChange={(e) =>
                  handleUserChange(
                    e.target.value,
              ...
              >
                <option value="">
                  Selecione o usuário responsável pelo agendamento
                </option>
                {users.map((user) => (
                  <option key={user.id} value={user.name} data-id={user.id}>
                    {user.name}
                  </option>
                ))}
              </select>
